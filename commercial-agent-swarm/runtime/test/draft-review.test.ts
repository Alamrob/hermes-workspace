...
describe('internal draft review gate', () => {
  ...
  it('defines idempotent A2 commands and never creates approvals, recipients, mail, CRM, or external eligibility', async () => {
    ...
    assert.match(sql, /jsonb_array_length\(result_envelope#>'\{agent_result,actions_taken\}'\)=0/)
    assert.match(sql, /jsonb_array_length\(result_envelope#>'\{agent_result,external_changes\}'\)=0/)
    assert.match(sql, /external_actions[^\n]+IS NULL OR[^\n]+external_actions[^\n]+integer=0/)
    assert.match(sql, /\[\[:cntrl:\]\]/)
    assert.match(sql, /\[\|\]/)
    assert.match(sql, /CLP 1\[\.\]800\[\.\]000/)
