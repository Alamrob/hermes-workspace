...
describe('Paperclip commercial automation', () => {
  ...
  it('dispatches ALA-52 only from signed completed ALA-51 evidence and keeps every draft internal and ineligible', async () => {
    ...
    assert.match(admissionPlan.assignments[0].instruction, /exactly the top-level keys status and reviews/)
    assert.match(admissionPlan.assignments[0].instruction, /slot, company, url, source_state, decision, reason, risk_flags, source_draft_sha256, approval_state, external_action_eligible/)
    assert.match(admissionPlan.assignments[0].instruction, /reason must be concise, non-empty and single-line/)
