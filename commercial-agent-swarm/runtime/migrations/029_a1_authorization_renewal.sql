...
ALTER TABLE control.a1_research_authorizations
  DROP CONSTRAINT a1_research_authorizations_review_id_key;

ALTER TABLE control.a1_research_authorizations
  ADD COLUMN supersedes_authorization_id uuid NULL
    REFERENCES control.a1_research_authorizations(authorization_id) ON DELETE RESTRICT,
  ADD CONSTRAINT a1_research_authorizations_supersedes_key UNIQUE(supersedes_authorization_id),
  ADD CONSTRAINT a1_research_authorizations_not_self_superseding
    CHECK(supersedes_authorization_id IS NULL OR supersedes_authorization_id<>authorization_id);

CREATE INDEX a1_research_authorizations_review_latest_idx
  ON control.a1_research_authorizations(review_id,reviewed_at DESC,created_at DESC,authorization_id DESC);

ALTER TABLE control.a1_research_order_authorizations
  DROP CONSTRAINT a1_research_order_authorizations_review_id_key;

CREATE INDEX a1_research_order_authorizations_review_latest_idx
  ON control.a1_research_order_authorizations(review_id,reviewed_at DESC,created_at DESC,order_authorization_id DESC);
...
BEGIN
  ...
  FROM control.a1_research_authorizations
  WHERE review_id=$1
  ORDER BY reviewed_at DESC,created_at DESC,authorization_id DESC
  LIMIT 1;
  IF FOUND THEN
  ...
  next_gate:=CASE
    ...
    WHEN authorization_row.expires_at<=observed_at THEN 'authorization_expired'
...
DECLARE
  ...
  latest control.a1_research_authorizations%ROWTYPE;
  dossier jsonb;
  now_at timestamptz:=clock_timestamp();
BEGIN
  PERFORM pg_advisory_xact_lock(hashtextextended('a1-research-auth:review:'||$2::text,0));
  PERFORM pg_advisory_xact_lock(hashtextextended('a1-research-auth:key:'||$11,0));
  SELECT * INTO existing FROM control.a1_research_authorizations
  WHERE authorization_id=$1 OR idempotency_key=$11;
  IF FOUND THEN
  ...
  IF $3 NOT IN('approved','rejected') OR length(btrim($4)) NOT BETWEEN 20 AND 1000 OR
     ...
     abs(extract(epoch FROM now_at-$7))>300 OR $8<=$7 OR $8>$7+interval '30 minutes' OR $8<=now_at OR
     $9!~'^[0-9a-f]{64}$' OR
  ...
  ORDER BY reviewed_at DESC,created_at DESC,authorization_id DESC
  LIMIT 1;
  IF FOUND AND (latest.expires_at>now_at OR $7<latest.expires_at)
  THEN RAISE EXCEPTION 'A1_RESEARCH_AUTHORIZATION_ACTIVE_CONFLICT'; END IF;
  ...
  IF dossier->>'status'<>'authorization_required' OR dossier->>'reviewCompleted'<>'true' OR
     ...
     dossier->>'productionGate'<>'blocked' OR (dossier->>'externalActions')::integer<>0 OR
     EXISTS(
       SELECT 1 FROM control.missions mission
       WHERE mission.payload#>>'{metadata,a1_research_review_id}'=$2::text
     )
  ...
  INSERT INTO control.a1_research_authorizations(
    ...
    dossier_sha256,attestations,idempotency_key,request_sha256,supersedes_authorization_id
  ) VALUES(
    $1,$2,$3,btrim($4),$5,$6,$7,$8,$9,$10,$11,$12,
    CASE WHEN latest.authorization_id IS NULL THEN NULL ELSE latest.authorization_id END
  );
  INSERT INTO control.audit_events(event) VALUES(jsonb_build_object(
    'event',CASE WHEN latest.authorization_id IS NULL THEN 'a1_research_authorization_recorded' ELSE 'a1_research_authorization_renewed' END,
    'authorization_id',$1,'review_id',$2,'decision',$3,
    ...
    'recorded_at',to_char(now_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"')
  ));
  RETURN control.build_a1_research_authorization_state($2,$9);
END$$;
...
BEGIN
  PERFORM pg_advisory_xact_lock(hashtextextended('a1-research-auth:review:'||$2::text,0));
  PERFORM pg_advisory_xact_lock(hashtextextended('a1-order-auth:key:'||$15,0));
  SELECT * INTO existing FROM control.a1_research_order_authorizations
  WHERE order_authorization_id=$1 OR parent_authorization_id=$3 OR mission_id=$12 OR
        unsigned_work_order_sha256=$11 OR user_authorization_sha256=$13 OR idempotency_key=$15;
  IF FOUND THEN
  ...
  IF $4 NOT IN('approved','rejected') OR length(btrim($5)) NOT BETWEEN 20 AND 1000 OR
     ...
     abs(extract(epoch FROM now_at-$8))>300 OR $9<=$8 OR $9>$8+interval '30 minutes' OR $9<=now_at OR
  ...
  LIMIT 1
  FOR SHARE;
  IF NOT FOUND OR parent.authorization_id<>$3 OR parent.decision<>'approved' OR
     parent.dossier_sha256<>$10 OR parent.expires_at<=now_at OR $8<parent.reviewed_at OR $9>parent.expires_at
