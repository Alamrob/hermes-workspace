...
CREATE TABLE IF NOT EXISTS control.dispatch_jobs(
 ...
 mission_usage_value_ceiling_usd numeric(14,6) NOT NULL CHECK(mission_usage_value_ceiling_usd>0),
 usage_value_reservation_usd numeric(14,6) NOT NULL CHECK(usage_value_reservation_usd>0),
 usage_value_consumed_usd numeric(14,6) NOT NULL DEFAULT 0 CHECK(usage_value_consumed_usd>=0),
 ...
 usage_value_actual_usd numeric(14,6), cash_cost_actual_usd numeric(14,6) NOT NULL DEFAULT 0 CHECK(cash_cost_actual_usd=0), pricing_snapshot_id text, tokens_used bigint, api_calls_used integer, error text,
...
BEGIN
 ...
 SELECT coalesce(sum(usage_value_reservation_usd),0)INTO active_reserved FROM control.dispatch_jobs WHERE mission_id=$2 AND status='queued';SELECT coalesce(sum(usage_value_consumed_usd),0)INTO spent FROM control.dispatch_jobs WHERE mission_id=$2;
 initial_status:=CASE WHEN ceiling-spent-active_reserved<$9 THEN'budget_exceeded'ELSE'queued'END;
 INSERT INTO control.dispatch_jobs(job_id,mission_id,trace_id,idempotency_key,profile_id,instruction,evidence,status,mission_usage_value_ceiling_usd,usage_value_reservation_usd,maximum_tokens,maximum_api_calls,max_attempts,created_at,updated_at)
 ...
 IF existing.job_id=$1 AND existing.trace_id=$3 AND existing.profile_id=$5 AND existing.instruction=$6 AND existing.evidence=jsonb_build_object('trust','untrusted_data','content',$7) AND existing.usage_value_reservation_usd=$9 AND existing.maximum_tokens=$10 AND existing.maximum_api_calls=$11 AND existing.max_attempts=$12 AND stored_dependencies=requested_dependencies THEN RETURN existing.job_id;END IF;
...
DECLARE n integer;now_at timestamptz:=clock_timestamp();BEGIN
 WITH changed AS(UPDATE control.dispatch_jobs SET status='usage_unknown',lease_owner=NULL,lease_until=NULL,child_timeout_seconds=NULL,next_attempt_at=now_at,updated_at=now_at,error='LEASE_EXPIRED_USAGE_UNKNOWN' WHERE status='leased'AND lease_until<=now_at RETURNING job_id,status)
...
DECLARE selected uuid;now_at timestamptz:=clock_timestamp();BEGIN
 ...
 UPDATE control.dispatch_jobs SET status='leased',lease_owner=$1,lease_until=now_at+make_interval(secs=>$2),child_timeout_seconds=$3,attempts=attempts+1,usage_value_consumed_usd=usage_value_consumed_usd+usage_value_reservation_usd,updated_at=now_at WHERE job_id=selected;
...
CREATE OR REPLACE FUNCTION control.fail_dispatch(uuid,text,text,boolean,text) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,control AS $$
DECLARE target text;now_at timestamptz:=clock_timestamp();job control.dispatch_jobs%ROWTYPE;BEGIN
 ...
 target:=CASE WHEN $5='usage_unknown'THEN'usage_unknown'WHEN $4 AND job.attempts<job.max_attempts THEN'queued'ELSE'failed'END;
 UPDATE control.dispatch_jobs SET status=target,lease_owner=NULL,lease_until=NULL,child_timeout_seconds=NULL,next_attempt_at=now_at,usage_value_consumed_usd=CASE WHEN $5='not_started'THEN greatest(0,usage_value_consumed_usd-usage_value_reservation_usd)ELSE usage_value_consumed_usd END,error=left($3,1000),updated_at=now_at WHERE job_id=$1;
...
DECLARE target text;now_at timestamptz:=clock_timestamp();job control.dispatch_jobs%ROWTYPE;BEGIN
 ...
 IF $3->'usage'->'cost'->>'source'<>'official_docs_snapshot' OR coalesce($3->'usage'->'cost'->>'pricing_snapshot_id','')='' OR ($3->'usage'->'cost'->>'cash_cost_usd')::numeric<>0 THEN RAISE EXCEPTION'INVALID_PRICING_AUTHORITY';END IF;
 SELECT*INTO job FROM control.dispatch_jobs WHERE job_id=$1 AND status='leased'AND lease_owner=$2 AND lease_until>now_at FOR UPDATE;
 IF NOT FOUND THEN RAISE EXCEPTION'DISPATCH_LEASE_CONFLICT';END IF;
...
REVOKE ALL ON control.dispatch_jobs,control.dispatch_dependencies,control.dispatch_events FROM PUBLIC,commercial_runtime,commercial_work_order_ingestor,commercial_approver,commercial_safety_operator,commercial_observer;
REVOKE ALL ON SEQUENCE control.dispatch_events_event_id_seq FROM PUBLIC,commercial_runtime,commercial_work_order_ingestor,commercial_approver,commercial_safety_operator,commercial_observer;
REVOKE ALL ON FUNCTION control.enqueue_dispatch(uuid,uuid,uuid,text,text,text,text,uuid[],numeric,bigint,integer,integer),control.recover_dispatch_leases(),control.claim_dispatch(text,integer,integer),control.fail_dispatch(uuid,text,text,boolean,text),control.complete_dispatch(uuid,text,jsonb,text,numeric,bigint,integer) FROM PUBLIC,commercial_runtime,commercial_work_order_ingestor,commercial_approver,commercial_safety_operator,commercial_observer;
GRANT EXECUTE ON FUNCTION control.enqueue_dispatch(uuid,uuid,uuid,text,text,text,text,uuid[],numeric,bigint,integer,integer),control.recover_dispatch_leases(),control.claim_dispatch(text,integer,integer),control.fail_dispatch(uuid,text,text,boolean,text),control.complete_dispatch(uuid,text,jsonb,text,numeric,bigint,integer) TO commercial_runtime;
COMMIT;
