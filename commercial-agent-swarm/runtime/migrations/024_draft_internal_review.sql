...
BEGIN
  SELECT count(*)=2 AND bool_and(status='succeeded' AND usage_budget_state='settled' AND api_calls_used=1 AND
    jsonb_typeof(result_envelope#>'{agent_result,actions_taken}')='array' AND jsonb_array_length(result_envelope#>'{agent_result,actions_taken}')=0 AND
    jsonb_typeof(result_envelope#>'{agent_result,external_changes}')='array' AND jsonb_array_length(result_envelope#>'{agent_result,external_changes}')=0 AND
    ((result_envelope#>'{agent_result,metrics,external_actions}') IS NULL OR (result_envelope#>>'{agent_result,metrics,external_actions}')::integer=0)) AND
