...
export class PostgresRuntimeRepository implements RuntimeRepository {
  ...
  async getA1DispatchExecutionArmState(missionId:string):Promise<A1DispatchExecutionArmState|null>{
    const result=await this.pool.query<{state:unknown}>('SELECT control.get_a1_dispatch_execution_arm($1::uuid) AS state',[missionId]);const state=result.rows[0]?.state;return state===null||state===undefined?null:validateA1DispatchExecutionArmState(state)
  }
